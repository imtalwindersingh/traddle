  ...
  switch (action.type) {
    case FETCH_POSTLISTING_SUCCESS: {
      ...
      // console.log('prevStatepkglist:',state,postList);
        if(action.listtype == 'bookmark' || action.listtype == 'myprofile')
    ...
    //   var tmpdataSource=state.postList;
    //   var list=null;
    //   console.log("source",tmpdataSource);   
    //   if(tmpdataSource!=null){
    //   if(tmpdataSource.length > 0){
    //     for (var i = 0;i < state.postList.length ;i++) {
    //       if(tmpdataSource[i]['_id'] == action._id)
    //       {
    //           tmpdataSource.splice(i,1);
    //           var list=tmpdataSource;      
    //         CommonUtility.showToast('Post deleted successfully');
    //       }
